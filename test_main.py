...
class TestChessGame:
    ...
    def test_pawn_promotion(self, setup_board):
        ...
        setup_board.clear()
        setup_board.add_piece(Pawn(Color.WHITE, "a7"))
        setup_board.active_player = Color.WHITE
        ...
        assert setup_board['a8'].color == Color.WHITE

    def test_en_passant_capture(self, setup_board):
        # Simulate the conditions for en passant
        setup_board.clear()
        setup_board.add_piece(Pawn(Color.WHITE, "e5"))
        setup_board.add_piece(Pawn(Color.BLACK, "d7"))
        setup_board.active_player = Color.BLACK
        setup_board.move('d7', 'd5')
        # Perform en passant capture
        setup_board.active_player = Color.WHITE
        setup_board.move('e5', 'd6')
        assert setup_board['d6'] == Pawn(Color.WHITE, 'd6')
        assert setup_board['d5'] is None
        assert isinstance(setup_board.captured_pieces[Color.BLACK][-1], Pawn)
    ...
    def test_castling(self, setup_board):
        ...
        assert isinstance(setup_board['e1'], Rook)

    def test_checkmate_detection(self, setup_board):
        # Setup a simple checkmate position
        setup_board.clear()
        setup_board.add_piece(King(Color.BLACK, "a8"))
        setup_board.add_piece(Queen(Color.WHITE, "b6"))
        setup_board.add_piece(King(Color.WHITE, "c6"))
        setup_board.active_player = Color.WHITE
    ...
    def test_stalemate_detection(self, setup_board):
        ...
        setup_board.clear()
        setup_board.add_piece(King(Color.BLACK, "a8"))
        setup_board.add_piece(Queen(Color.WHITE, "c7"))
        setup_board.add_piece(King(Color.WHITE, "c6"))
        setup_board.active_player = Color.BLACK
        setup_board.move('a8', 'a7')  # Black king has no legal moves but is not in check
        black_king = setup_board.get_king(Color.BLACK)
    ...
    def test_basic_capture_mechanics(self, setup_board):
        setup_board.add_piece(piece=Pawn(Color.BLACK, location="b3"))  # Setup a black pawn
        ...
        assert isinstance(setup_board.captured_pieces[Color.BLACK][0], Pawn)
        assert setup_board.captured_pieces[Color.BLACK][0].color == Color.BLACK
    ...
    def test_extended_capture_mechanics(self, setup_board):
        ...
        setup_board.add_piece(Pawn(Color.BLACK, "b3"))
        setup_board.add_piece(Pawn(Color.BLACK, "c3"))
        setup_board.add_piece(Pawn(Color.BLACK, "d3"))
        setup_board.add_piece(Pawn(Color.BLACK, "e3"))

        # Setting up other black pieces
        setup_board.add_piece(Knight(Color.BLACK, "c6"))
        setup_board.add_piece(Bishop(Color.BLACK, "f6"))

        # White pawn captures black pawn
        white_pawn = setup_board['a2']
        setup_board.move('a2', 'b3')
        assert white_pawn.location == 'b3' and isinstance(setup_board.captured_pieces[Color.BLACK][0], Pawn) and setup_board.captured_pieces[Color.BLACK][0].color == Color.BLACK
        ...
        assert pawn_b2.location == 'c3' and isinstance(setup_board.captured_pieces[Color.BLACK][1], Pawn) and setup_board.captured_pieces[Color.BLACK][1].color == Color.BLACK

        # White knight capturing black pawn
        setup_board.add_piece(Knight(Color.WHITE, "d5"))
        knight_d5 = setup_board['d5']
        setup_board.active_player = Color.WHITE
        setup_board.move('d5', 'e3')
        assert knight_d5.location == 'e3' and isinstance(setup_board.captured_pieces[Color.BLACK][2], Pawn) and setup_board.captured_pieces[Color.BLACK][2].color == Color.BLACK
    ...
    def test_tempmove(self):
        ...
        with Board.TempMove(board):
            board.move("b2", "b4")
            assert board['b4'] == Pawn(Color.WHITE, 'b4')
            assert board['b2'] is None

            board.move("e7", "e5")
            assert board['e5'] == Pawn(Color.BLACK, 'e5')
            assert board['e7'] is None

            board.move("g1", "f3")
            assert board['f3'] == Knight(Color.WHITE, 'f3')
            assert board['g1'] is None

            # Bishop takes Pawn
            board.move("f8", "b4")
            assert board['b4'] == Bishop(Color.BLACK, 'b4')
            assert board['f8'] is None

            assert board.captured_pieces == {Color.WHITE: [Pawn(Color.WHITE, None)], Color.BLACK: []}
            assert len(board.pieces[Color.WHITE]) == 15

        assert board['b2'] == Pawn(Color.WHITE, 'b2')
        assert board['b4'] is None
        assert board.captured_pieces == {Color.WHITE: [], Color.BLACK: []}
